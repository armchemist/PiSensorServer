...
//
// ⚠️ 임시 1점 보정이다. 2026-08-16, 수돗물을 pH 7.00 으로 가정하고 절편만
//    맞췄다 (측정 전압 1.9233V → 7.00).
//
//    기울기 3.5 는 검증하지 않은 이론값이다. 1점 보정은 기준점 근처에서만
//    맞고 pH 7 에서 멀어질수록 오차가 커진다. 게다가 수돗물은 완충 용액이
//    아니라 지역·시간·수온에 따라 6.5~8.5 로 변하고 공기 중 CO2 를 흡수하며
//    산성화되므로, 기준값 7.00 자체가 부정확하다.
//
//    실제 측정에 쓰려면 pH 4.00 / 7.00 표준액으로 2점 보정할 것.
//    절차는 sensors/README.md 참고.
const float PH_SLOPE     = 3.5;     // 미검증 (이론값)
const float PH_INTERCEPT = 0.268;   // 수돗물 1점 보정 (임시)
