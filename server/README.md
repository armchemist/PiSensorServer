...
| `POST` | `/rail/stop` | **이동 즉시 중단.** 위치를 잃는다 |
...
`POST /rail/stop` 은 진행 중인 이동을 즉시 끊는다. 이동은 서버에서 블로킹으로
돌아가지만, 요청마다 스레드가 붙으므로 이동 중에도 처리된다.

⚠️ **멈추면 위치를 잃는다.** 전송을 끊는 순간 몇 펄스가 나갔는지 알 수 없기
때문이다. 이후 이동은 `/rail/set_position` 으로 실제 위치를 알려줘야 풀린다.
`/rail/resume` 은 저장된 값을 그대로 쓰겠다는 뜻이라 이 경우에 맞지 않는다.

중단된 이동 요청은 `409` 로 끝난다. 사람이 일부러 멈춘 것이지만 요청한 이동은
완료되지 않았으므로 성공으로 처리하면 안 된다.
